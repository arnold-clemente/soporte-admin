<template>
  <AppLayout>
    ...
    <div class="w-full flex items-center justify-end px-6">
      <div class="ml-4 px-4 py-2 bg-red-700 hover:bg-red-600 transition-all rounded-md text-white font-bold">
        <Link :href="route('entrega.create')" class="flex items-center">
      ...
      <table class="table table-hover table-bordered table-striped" id="sampleTable">
        ...
        <tbody>
          <tr v-for="entrega in entregas" :key="entrega.id">
            <td>
              <a :href="route('entrega.reporte', entrega)" target="_blank"
            ...
            <td>
              {{
                entrega.funcionario.nombres +
                " " +
                entrega.funcionario.apellidos
              }}
            </td>
            <td>{{ entrega.nota.cite }}</td>
          </tr>
        </tbody>
      </table>
    </div>
